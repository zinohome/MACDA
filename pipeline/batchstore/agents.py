...
async def store_signal(stream):
    ...
    async for datas in stream.take(10000, within=settings.TSDB_BATCH_TIME):
        ...
        # Predict
        for data in datas:
            log.debug("==========---------- Get predict data in batch ==========----------")
            dev_mode = settings.DEV_MODE
            mode = 'dev'
            if not dev_mode:
                mode = 'pro'
            ...
            if predictdata['len'] > 0:
                # ref leak predict
                ref_leak_u11 = sp.polyfit(predictdata['data']['dvc_w_op_mode_u1'], predictdata['data']['dvc_i_fat_u1'],
                                          predictdata['data']['dvc_w_freq_u11'],
                                          predictdata['data']['dvc_i_suck_pres_u11'])
                ref_leak_u12 = sp.polyfit(predictdata['data']['dvc_w_op_mode_u1'], predictdata['data']['dvc_i_fat_u1'],
                                          predictdata['data']['dvc_w_freq_u12'],
                                          predictdata['data']['dvc_i_suck_pres_u12'])
                ref_leak_u21 = sp.polyfit(predictdata['data']['dvc_w_op_mode_u2'], predictdata['data']['dvc_i_fat_u2'],
                                          predictdata['data']['dvc_w_freq_u21'],
                                          predictdata['data']['dvc_i_suck_pres_u21'])
                ref_leak_u22 = sp.polyfit(predictdata['data']['dvc_w_op_mode_u2'], predictdata['data']['dvc_i_fat_u2'],
                                          predictdata['data']['dvc_w_freq_u22'],
                                          predictdata['data']['dvc_i_suck_pres_u22'])
                # ref pump predict
                ref_pump_u1 = 0
                ref_pump_u2 = 0
